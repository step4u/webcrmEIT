...
public class RecordListReportDownload extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		try {
			...
			JSONArray recordList = JSONArray.fromObject(model.get("recordList"));
			...
			HSSFSheet sheet = wb.createSheet("녹취리스트");
			...
			sheet.setColumnWidth((short) 6, (short) 45 * 300);
			...
			*/
			...
			for (int c = 0; c <= 6; c++) {
				...
				cell.setCellValue("녹취리스트");
				cell.setCellStyle(hStyle);
			}
			sheet.addMergedRegion(new Region((short) rownum, (short) 0, (short) rownum, (short) 6));
			...
			cell.setCellValue("콜유형");
			cell.setCellStyle(Ti_T_C_T_style);

			cell = row.createCell(4);
			cell.setCellValue("녹취일시");
			cell.setCellStyle(Ti_T_C_T_style);
			...
			cell = row.createCell(1);
			cell.setCellValue(recordList.getJSONObject(i).getString("extensionNo"));
			cell.setCellStyle(C_D_C_T_style);
			
			cell = row.createCell(2);
			cell.setCellValue(recordList.getJSONObject(i).getString("telNo"));
			cell.setCellStyle(C_D_C_T_style);
			
			cell = row.createCell(3);
			cell.setCellValue(recordList.getJSONObject(i).getString("callTypCd"));
			cell.setCellStyle(C_D_C_T_style);
			
			cell = row.createCell(4);
			cell.setCellValue(recordList.getJSONObject(i).getString("recStartDate") + " " + recordList.getJSONObject(i).getString("recStartHms"));
			cell.setCellStyle(C_D_C_T_style);
			
			cell = row.createCell(5);
			cell.setCellValue(recordList.getJSONObject(i).getInt("fileSize"));
			cell.setCellStyle(C_N_R_T_style);
			
			cell = row.createCell(6);
			cell.setCellValue(recordList.getJSONObject(i).getString("fileName"));
			cell.setCellStyle(C_D_C_T_style);
			...
			logger.debug("녹취현황 다운로드 완료.");
			
			String fileName = EitDateUtil.getToday()
					+ EitDateUtil.getTotime() + "_녹취현황.xls";
